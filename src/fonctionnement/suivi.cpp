...
/**** Fonction SemiFiltreParticulaire ****/
/**** VERSION 2 ***/
Point suivi::semiFiltreParticulaire(Mat imgInput, Piece piece){
    //Passage de l'image en noir & blanc
    Mat img;
    cvtColor(imgInput,img,CV_RGB2GRAY);

    //------------------------------
    // Initialisation des variables:
    //------------------------------
	int nbPoint=20,nbessai=10,MSE,sigma=10;
    Point XY[nbPoint]; // Points pris au hasard
    float P[nbPoint];  // Probabilité pour chaque points XY
    ...
    Point PFIN[nbessai]; //Probabilité pour chaque centreEstimé
	Piece pc2; // Pièce pour test comparaison getMSE

	//-------------------------------------
	// Lancement pour un nb d'essai
	// de l'algorithme pour avoir un
	// centre estimée
	//-------------------------------------
    ...
    for(int i=0;i<nbessai;i++){
        ...
        for(int i=0;i<nbPoint;i++){
            ...
            MSE = getMSE(piece,pc2); // Moyenne pondéré, comparaison entre les pièces
    ...
    //------------------------------------------//
    // Selection du centre estimé final         //
    // en fonction des anciens centres estimés  //
    //------------------------------------------//
        ...
        //Marque le centre estimé final
        drawMarker(imgInput, Point(sommeX,sommeY), Scalar(255),MARKER_CROSS,20,4,8);
...
/**** VERSION 3 ****/
Point suivi::filtreParticule(Mat imgInput,int nblancement,Piece piece){
    Mat img;
    cvtColor(imgInput,img,CV_RGB2GRAY);
    Point XY[35];
    float P[35];
    ...
    //pour le reste des images
        ...
        for(int i=0; i<nbPoint2 ; i++){ //nbpoint
            ...
            drawMarker(imgInput, Point(XY2[i].x, XY2[i].y), Scalar(255),MARKER_CROSS,20,1,8);
            //Marque des nouveaux points (noirs)
            drawMarker(imgInput, Point(XY3[i].x, XY3[i].y), Scalar(0),MARKER_CROSS,20,1,8);
...
void suivi::suiviSemiFiltreParticulaire(){
    int nbImg=1;
    if(!Systeme::listePieces.empty()){
       for (Mat &img : Systeme::listeImages){
            if(nbImg!=1){
               int i=1;
                std::cout<<"Image numero "<<nbImg<<std::endl;
                for (Piece &piece : Systeme::listePieces){
                        Point pos = suivi::semiFiltreParticulaire(img,piece);
                        piece.setCoord(pos);
                        std::cout<<"La piece numero "<<i<<" a bouge a la position: "<< pos<<std::endl;
                        i++;
                }
            }
            nbImg++;
        }
    }else std::cout<<"Pas de piece à suivre"<<endl;
}

void suivi::suiviFiltreParticulaire(){
       int nbfois=0;
       for (Mat &img : Systeme::listeImages){
       	for (Piece &piece : Systeme::listePieces) // access by reference to avoid copying
	        {
	            Point pos = suivi::filtreParticule(img,nbfois,piece);
	            piece.setCoord(pos);
	            std::cout<<"La piece a bouge a la position: "<< pos<<std::endl;
	        }
        nbfois++;
       }
}
